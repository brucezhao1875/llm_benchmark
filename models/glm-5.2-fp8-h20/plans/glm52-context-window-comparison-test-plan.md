...
```text
同一模型、同一硬件、同一 vLLM 版本下，服务启动时的 max-model-len 设置为 32K、256K、1M，对短请求、长请求、吞吐、延迟、KV cache、启动时间和并发能力分别有什么影响。
```

重点不再只是验证“1M 能不能跑”，而是验证：

1. `启动为 32K` 是否能显著改善短请求吞吐和延迟。
2. `启动为 1M` 是否会影响普通短请求性能。
3. `启动为 1M` 后是否真的可以完成 1M 上下文请求。
4. 1M 请求的 TTFT、Prefill tokens/s、KV cache 占用、GPU 利用率是否可接受。
5. Gateway 是否必须按上下文长度路由到不同服务池。
...
1. 建立短上下文在线服务池 baseline。
2. 观察 32K 配置下启动时间、显存水位、KV cache 可用量。
3. 验证短请求吞吐和流式体验是否优于 256K/1M 配置。
...
1. 验证 1M 上下文服务能否成功启动。
2. 验证普通短请求在 1M 启动配置下是否退化。
3. 验证 1M 输入请求是否能成功完成。
4. 评估 1M 配置是否适合作为默认服务，还是只能作为 ultra-long 专用池。
...
| 指标 | 说明 |
|---|---|
| systemd restart 发起时间 | 判断总启动耗时 |
| `/v1/models` ready 时间 | 判断服务可用时间 |
| vLLM 日志关键阶段 | 包括 load、profile、compile、warmup |
| GPU 显存占用 | 空载状态显存水位 |
| KV cache blocks / usage | 如果 metrics 中可见，记录 KV cache 规划结果 |
| 启动是否失败 | 记录错误信息和失败阶段 |

启动时间记录格式：

```text
restart_issued_at=<timestamp>
ready_at=<timestamp>
ready_after_seconds=<seconds>
```

## 4. 32K 启动上下文测试矩阵

32K 配置只测试短上下文和普通 RAG，不测试超过 32K 的输入。

### 4.1 Decode 短请求矩阵

| 输入长度 | 输出长度 | request rate | 目的 |
|---:|---:|---:|---|
| 1K | 128 | 1 / 2 / 4 / 8 | 短回复低延迟 |
| 1K | 512 | 1 / 2 / 4 / 8 | 普通问答主场景 |
| 1K | 1024 | 1 / 2 / 4 | 中长回复 |
| 8K | 512 | 1 / 2 / 4 | RAG 主场景 |
| 8K | 1024 | 1 / 2 | RAG 长回复 |
...
```text
32K 启动配置下，不应提交超过 max-model-len 的请求。
```

## 5. 1M 启动上下文测试矩阵

1M 配置分成两部分：短请求退化测试和 1M 能力探针。

### 5.1 短请求退化测试

目的：验证 1M 启动配置是否影响普通在线请求。

| 输入长度 | 输出长度 | request rate | 对比对象 |
|---:|---:|---:|---|
| 1K | 128 | 1 / 2 / 4 / 8 | 对比 32K 与 256K |
| 1K | 512 | 1 / 2 / 4 / 8 | 对比 32K 与 256K |
| 1K | 1024 | 1 / 2 / 4 | 对比 32K 与 256K |
| 8K | 512 | 1 / 2 / 4 | 对比 32K 与 256K |
| 8K | 1024 | 1 / 2 | 对比 32K 与 256K |

判断重点：

1. output tokens/s 是否下降。
2. TTFT p95/p99 是否升高。
3. TPOT/ITL 是否变差。
4. max concurrent requests 是否下降。
5. vLLM waiting requests 是否更容易出现。

### 5.2 1M 能力探针

目的：以最小代价验证 1M 上下文能力，不做大矩阵。

| 上下文长度 | 输出长度 | 并发 | 超时 | 目的 |
|---:|---:|---:|---:|---|
| 384K | 1 | 1 | 10 min | 超过 256K 后趋势 |
| 512K | 1 | 1 | 15 min | 中间点 |
| 768K | 1 | 1 | 25 min | 接近 1M |
| 1M | 1 | 1 | 40 min | 1M 极限能力 |
| 1M | 16 | 1 | 45 min | 1M 短输出完整体验 |
| 1M | 1 | 2 | 60 min | KV cache 并发探针，可选 |

执行规则：

1. 必须按 384K、512K、768K、1M 逐级执行。
2. 任一阶段出现 OOM、服务崩溃、长时间无响应，则停止后续更高档位。
3. 每个测试点必须设置硬超时。
4. 1M 并发 2 是可选项，只有 1M 并发 1 成功且 KV cache 水位可接受时才执行。
...
| 指标 | 说明 |
|---|---|
| completed / failed | 成功和失败请求数 |
| output tokens/s | 主吞吐指标 |
| total tokens/s | 总 token 处理能力 |
| request throughput | 业务画像下的派生 QPS |
| TTFT p50/p95/p99 | 首 token 延迟 |
| TPOT p50/p95/p99 | 每输出 token 时间 |
| ITL p50/p95/p99 | 流式 token 间隔 |
| E2E p50/p95/p99 | 端到端耗时 |
| max concurrent requests | 最大在途请求数 |

### 6.2 服务端指标

| 指标 | 来源 | 说明 |
|---|---|---|
| GPU SM util | `nvidia-smi dmon` | GPU 是否打满 |
| GPU memory used | `nvidia-smi` | 显存占用 |
| GPU memory util | `nvidia-smi dmon` | memory activity，不等同显存占用 GB |
| power draw | `nvidia-smi dmon` | GPU 功耗 |
| running requests | vLLM `/metrics` | 正在执行请求数 |
| waiting requests | vLLM `/metrics` | 排队请求数 |
| KV cache usage | vLLM `/metrics` | KV cache 水位 |
| prefill time | vLLM `/metrics` | Prefill 耗时 |
| decode time | vLLM `/metrics` | Decode 耗时 |
...
| max-model-len | ready 时间 | 空载显存 | KV cache 规划 | 是否启动成功 |
|---:|---:|---:|---:|---|
| 32K | TBD | TBD | TBD | TBD |
| 256K | 已有历史数据 | 已有历史数据 | TBD | 已成功 |
| 1M | TBD | TBD | TBD | TBD |
...
```text
32K 启动配置 vs 256K 启动配置 vs 1M 启动配置
```
...
7. KV cache usage。
8. 是否 OOM。
9. 是否出现 waiting requests。
10. 是否适合在线同步调用。

## 8. 时间控制策略

为了避免 1M 测试耗时不可控，执行时必须遵守：

1. 所有 1M 能力探针必须设置超时。
2. 先跑短输出，不跑 512/1024/2048 长输出。
3. 先跑并发 1，不直接跑并发 2 或 4。
4. 每个阶段结束后检查服务是否仍 active。
5. 如果服务崩溃或 OOM，停止后续更高压力测试。
6. 测试脚本必须后台运行，日志落盘，减少 SSH 登录频率。

## 9. Gateway 功能要求补充

本测试与 Gateway 设计直接相关。若测试证明 32K、256K、1M 启动配置在短请求和长请求上差异明显，则 Gateway 必须支持模型流量调度能力。

新增 Gateway 能力要求：

| 能力 | 说明 |
|---|---|
| token 长度估算 | 请求进入后端前估算 input tokens |
| output tokens 估算 | 读取 `max_tokens` 或设置默认输出预算 |
| context tier 分类 | short / standard / long / ultra-long |
| decode tier 分类 | short-output / normal-output / long-output |
| pool routing | 路由到 32K、256K、1M 等不同服务池 |
| admission control | 按 token、用户、池子容量控制是否接收请求 |
| async long-context job | 1M 请求可转异步任务 |
| per-pool billing | 32K、256K、1M 不同计价 |
| per-pool observability | 每个池单独统计吞吐、延迟、错误率、成本 |

推荐路由原则：

```text
input <= 8K 且 output <= 512       -> 32K low-latency pool
input <= 64K 且 output <= 2048     -> 256K standard/agent pool
input > 256K 或需要 1M             -> 1M ultra-long pool
output 很长但 input 很短            -> long-decode pool 或 batch pool
1M 请求默认低并发、可异步、单独计费
```
...
1. 保存当前 256K baseline 数据作为历史对照。
2. 切换到 32K 启动配置。
3. 记录 32K 启动时间和空载状态。
4. 执行 32K 短请求矩阵和 32K 边界探针。
5. 切换到 1M 启动配置。
6. 记录 1M 启动时间和空载状态。
7. 执行 1M 短请求退化测试。
8. 执行 1M 能力探针，逐级从 384K 到 1M。
9. 如 1M 能力探针失败，记录失败点并停止更高压力测试。
10. 汇总生成 `32K vs 256K vs 1M` 对比报告。
11. 测试结束后，根据生产需要恢复到 baseline 或指定服务池配置。

## 2026-06-30 执行完成记录

本轮已执行上下文启动配置对比测试：

1. 32K 启动配置测试完成，ready 用时 220s，短请求矩阵和边界探针均成功。
2. 1M 启动配置在 30 分钟窗口内未 ready，记录为 `1m_start_failed`，未执行 1M 请求探针。
3. 测试结束后已恢复原始 256K baseline service。

最终报告路径：`/root/demo/docs/glm52-context-window-comparison-report-2026-06-30.md`。

远端结果目录：`/data/bench/glm52/20260630-061059-context-window-32k-vs-1m`。
